...
public class ElevatorStowCommand extends SequentialCommandGroup {
         ...
         * @param cleanPivot
         */
        public ElevatorStowCommand(CleanPivotModes mode, Elevator elevator, ManipJoint manipJoint,
                        CleanPivot cleanPivot) {
                addCommands(
                                ...
                                cleanPivot.runCleanerPivotCommand(mode)
                                                .alongWith(manipJoint.runCleanerPivotCommand(ManipJointPositions.STOW),
                                                                // when prev commands finish (instantaly since it they
                                                                // are RunCommands)
                                                                // sets elevator to stow angle only if the manipulator
                                                                // is near the stow angle
                                                                // this ensures that the manip doesnt hit anything while
                                                                // the elevator goes down
                                                                elevator.runElevatorCommand(ElevatorPositions.STOW)
                                                                                .onlyIf(() -> manipJoint
                                                                                                .getAngleSetpointGoal(
                                                                                                                ManipJointConstants.stow,
